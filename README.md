...
*浙江大学校园生活模拟器* 
## 邀请：关键词添加和补充
该游戏是一个基于大模型的文本游戏，world/keywords.json是大模型生成内容的一个重要根据，目前数据较为不全，欢迎有兴趣的校友们在其中补充关键信息（请参照原有json表结构）

## 项目简介
本项目为浙江大学校园生活模拟器，支持学业、选课、成就、事件、学期循环等玩法，后端基于 FastAPI，前端基于 HTML/JS，支持 Docker 部署和 Cloudflare Tunnel 公网分流。
目前，由于开发者@pirate-608 发现使用cloudflare tunnel免费版会出现websocket连接不稳定的问题，而其只有一个2vcpu2gib的小型服务器，无法支撑游戏的公网部署，故当前项目的公网服务暂不支持，期待更多爱好者愿意且有能力将该游戏发扬光大并部署到公网。
