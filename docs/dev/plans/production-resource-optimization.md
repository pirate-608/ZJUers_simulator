...
- MiniMax M2-her 使用共享 `httpx.AsyncClient`，并在 FastAPI shutdown 时关闭，减少频繁创建客户端和建连成本。
...
- MiniMax M2-her 使用共享 `httpx.AsyncClient`，并在 FastAPI shutdown 时关闭，减少频繁建连和文件描述符压力。（已执行）
