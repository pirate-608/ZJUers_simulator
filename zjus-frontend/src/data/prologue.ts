...
export const PROLOGUE_DEDICATION_LINES = [
  '谨以此游戏献给每一位灿若星辰的浙大人。',
  '这是一个关于你，我，和他们的故事。',
] as const

export const PROLOGUE_DIARY_PAGES = [
  [
...
export const PROLOGUE_LINES = [
  ...
  ...PROLOGUE_DIARY_PAGES.flat(),
] as const

const DIARY_START_INDEX = PROLOGUE_DEDICATION_LINES.length
const THRESHOLD_START_INDEX =
  DIARY_START_INDEX
  + PROLOGUE_DIARY_PAGES[0].length
  + PROLOGUE_DIARY_PAGES[1].length

export const PROLOGUE_FIRST_DIARY_INDEX = DIARY_START_INDEX

export const PROLOGUE_FINAL_PAGE_INDEX = THRESHOLD_START_INDEX
...
const PROLOGUE_SCENES: Array<{ from: number; scene: PrologueScene }> = [
  ...
  { from: DIARY_START_INDEX, scene: { image: imageUrl('zjg_autumn.jpg'), tone: 'morning' } },
  { from: DIARY_START_INDEX + 5, scene: { image: imageUrl('qizhen_lake.jpg'), tone: 'lake' } },
  { from: DIARY_START_INDEX + 7, scene: { image: imageUrl('sunset.webp'), tone: 'sunset' } },
  { from: DIARY_START_INDEX + 9, scene: { image: imageUrl('qizhen_lake.jpg'), tone: 'lake' } },
  { from: THRESHOLD_START_INDEX, scene: { image: imageUrl('qiushimen.webp'), tone: 'threshold' } },
]
